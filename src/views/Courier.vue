<template>
  <div class="courier">
    ...
    <div class="courier__container">
      <ul class="courier__description description">
        ...
        <template v-if="trackingInfo.ForwarderPhone">
          <li>
            <span class="description__title">
              <span> <i class="el-icon-phone"></i> </span>Телефон:</span
            >
            <a
              href="tel:`${trackingInfo.ForwarderPhone}`"
              class="description__data"
              >{{ trackingInfo.ForwarderPhone }}</a
            >
          </li>
        </template>
        <template v-if="trackingInfo.Cargos">
          <li v-for="(product, index) in trackingInfo.Cargos" :key="index">
            <span class="description__title">
              <span> <i class="el-icon-s-goods"></i> </span>Товар:</span
            >
            <span class="description__data"
              >{{ product.CargoNameCode }} - {{ product.CargoCount }} шт.</span
            >
          </li>
        </template>
      </ul>
    ...
    <div class="link-back">
      <router-link :to="`/?hash=${this.$route.query.hash}`"
        ><el-button size="small" round>Назад</el-button></router-link
      >
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getTrackingInfo() {
      ...
      if (hash) {
        this.$store.dispatch("getTracking", hash).then(() => {});
      } else {
        this.is404 = true;
      }
    },
